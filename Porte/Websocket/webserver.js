...
var LED26 = new Gpio(4, 'out'); //use GPIO pin 26 as output
var LED20 = new Gpio(4, 'out'); //use GPIO pin 20 as output
var LED21 = new Gpio(4, 'out'); //use GPIO pin 21 as output
var LED16 = new Gpio(4, 'out'); //use GPIO pin 16 as output
...
io.sockets.on('connection', function (socket) {// WebSocket Connection
    ...
    socket.emit('GPIO16', GPIO17value);
	...
	console.log('new GPIO16 value='+GPIO17value);
	LED16.writeSync(GPIO17value); //turn LED on or off
	console.log('Send new GPIO16 state to ALL clients');
	io.emit('GPIO16', GPIO17value); //send button status to ALL clients 	
    });
	...
	GPIO17value = data;
	if (GPIO17value != LED16.readSync()) { //only change LED if status has changed
	    LED16.writeSync(GPIO17value); //turn LED on or off
	    console.log('Send new GPIO16 state to ALL clients');
	    io.emit('GPIO16', GPIO17value); //send button status to ALL clients 
	};
